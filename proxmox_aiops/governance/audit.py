"""Unified audit logging engine — all Proxmox skills write to a single SQLite database.

Replaces 7 per-skill JSON Lines audit loggers with one shared ``~/.proxmox-aiops/audit.db``.
...
_log = logging.getLogger("proxmox-aiops.audit")

# Back-compat override hook. When None (default) the path resolves via
# ops_path("audit.db") (honoring PROXMOX_AIPROXMOX_HOME). Downstream code/tests may set this
# to a fixed Path to redirect the default DB — kept so the PROXMOX_AIPROXMOX_HOME refactor
