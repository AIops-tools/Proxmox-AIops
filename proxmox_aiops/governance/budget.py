...
2. **Runaway breaker (on by default).** The same ``(tool, params)`` called more
   than ``PROXMOX_RUNAWAY_MAX`` times within ``PROXMOX_RUNAWAY_WINDOW_SEC`` trips a
...
from proxmox_aiops.governance.policy import PolicyResult
...
class BudgetExceeded(Exception):
    ...
    Subclasses nothing Proxmox-specific; the decorator wraps it into a
