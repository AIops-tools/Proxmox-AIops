"""Encrypted secret storage for proxmox-aiops.

Secrets (API token secrets / login passwords) are NEVER written to disk in
plaintext. They live in ``~/.proxmox-aiops/secrets.enc`` encrypted with Fernet
...
The master password is resolved (in order) from:
  ...
  1. the env var ``PROXMOX_AIOPS_MASTER_PASSWORD`` (for non-interactive use:
...
def migrate_legacy_env(prefix: str, suffix: str, password: str | None = None) -> list[str]:
    ...
    Legacy keys look like ``PROXMOX_<TARGET>_SECRET``; ``prefix='PROXMOX_'`` and
    ``suffix='_SECRET'`` map them back to a target name. The plaintext ``.env``
    is renamed to ``.env.migrated`` (chmod 600) so nothing is silently lost.
    Returns the list of imported target names.
    """
