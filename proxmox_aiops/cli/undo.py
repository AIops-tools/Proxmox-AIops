"""``proxmox-aiops undo`` — list recorded undo tokens and apply them.
...
def undo_apply_cmd(
    ...
    if dry_run:
        ...
        # ``wouldApply`` is absent on a refusal; dry_run_preview catches that
        # before the banner prints, so the '?' fallback never stands in for a
        # tool name the caller was actually refused.
