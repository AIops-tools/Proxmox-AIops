"""``proxmox-aiops secret`` — manage the encrypted credential store.

Secrets are stored in ``~/.proxmox-aiops/secrets.enc`` (Fernet, key derived
from a master password via scrypt). Nothing here ever prints a secret value.
"""
...
from proxmox_aiops.config import SECRET_ENV_PREFIX, SECRET_ENV_SUFFIX
from proxmox_aiops.secretstore import (
    SECRETS_FILE,
    SecretStore,
    check_permissions,
    migrate_legacy_env,
...
NameArg = Annotated[str, typer.Argument(help="Target name the secret belongs to")]
...
def secret_set(
    ...
    """Store (or replace) a secret for a target — value is read hidden."""
...
def secret_list() -> None:
    """List target names that have a stored secret (values never shown)."""
    store = SecretStore.unlock()
    names = store.names()
    if not names:
        console.print("[yellow]No secrets stored yet. Add one: proxmox-aiops secret set <name>[/]")
        return
...
def secret_rotate_password() -> None:
    ...
    console.print("[green]✓ Master password rotated. Update PROXMOX_AIOPS_MASTER_PASSWORD.[/]")
