...
nothing touches the real ``~/.proxmox-aiops``.
"""
...
def test_roundtrip_set_get(store_dir):
    ...
    store = store.set("pve-lab", "token-uuid-123")
    reopened = ss.SecretStore.unlock("master-pw")
    assert reopened.get("pve-lab") == "token-uuid-123"


def test_value_not_plaintext_on_disk(store_dir):
    ss.SecretStore.unlock("master-pw").set("pve-lab", "super-secret-value")
...
def test_migrate_legacy_env(store_dir):
    (store_dir / ".env").write_text(
        "PROXMOX_PVE_LAB_SECRET=legacy-token\n# comment\nFOO=bar\n"
    )
    imported = ss.migrate_legacy_env("PROXMOX_", "_SECRET", "pw")
    assert imported == ["pve-lab"]
    assert ss.SecretStore.unlock("pw").get("pve-lab") == "legacy-token"
