...
### Changed
- **The ClawHub bundle plugin moved from `@aiops-tools/proxmox-aiops` to
  `@zw008/proxmox-aiops`**, matching the publisher the skill has always been under.
  ...
  `openclaw plugins install clawhub:@zw008/proxmox-aiops`. Nothing about the Python
...
- **The OpenClaw install path is documented.** The ClawHub bundle channel went
  ...
  `openclaw plugins install clawhub:@aiops-tools/proxmox-aiops`. States the `uvx`
...
- **Installable as a Claude Code plugin.** `.claude-plugin/plugin.json` plus a
  root `.mcp.json` make this repo a plugin, so `/plugin install proxmox-aiops@aiops-tools`
...
- **Installable from ClawHub as an OpenClaw bundle plugin** (`@aiops-tools/proxmox-aiops`): one install delivers the skill *and* its MCP
...
- **The skill was invisible to the model in OpenClaw.** Its metadata
  ...
  `anyBins: [proxmox-aiops, uvx]` — either one suffices — with every variable kept
...
### Fixed
- **Fixes a missing dependency that made the tool unusable as installed.** `proxmoxer` declares no dependencies at all, and its default `https` backend imports `requests` — so a clean install raised *"Chosen backend requires 'requests' module"* on the first connect and could not reach any server.
