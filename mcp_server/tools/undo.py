...
the inverse itself is destructive — carrying its own risk tier into the audit
row). ``undo_apply`` is itself governed; the inner tool it calls is audited the
...
def undo_apply(undo_id: str, dry_run: bool = False, target: Optional[str] = None) -> dict:
    ...
    The inverse runs through its own governed tool, so its own risk tier is
    recorded on the audit row there. Pass dry_run=True to preview the inverse
    call without executing it. A token can only be applied once.
